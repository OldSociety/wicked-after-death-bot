...
async function postRandomQuestion(channel) {
    try {
        ...
        const options = ['🇦', '🇧', '🇨', '🇩'];
        const answers = [randomQuestion.answer_1, randomQuestion.answer_2, randomQuestion.answer_3, randomQuestion.answer_4].filter(Boolean);
        const correctAnswerMapping = { 'A': '🇦', 'B': '🇧', 'C': '🇨', 'D': '🇩' };
        const correctAnswerEmoji = correctAnswerMapping[randomQuestion.correct_answer];

        const questionEmbed = new EmbedBuilder()
            .setColor(0x0099FF)
            .setTitle('Random Question Time!')
            .setDescription(randomQuestion.question_text)
            .addFields({ name: 'Options', value: answers.map((answer, index) => `${options[index]} ${answer}`).join('\n'), inline: true })
            .setTimestamp();

        const questionMessage = await channel.send({ embeds: [questionEmbed] });
        options.slice(0, answers.length).forEach(async option => await questionMessage.react(option));
