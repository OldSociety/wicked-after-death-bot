...
async function checkAccount(user) {
  ...
  const account = await User.findOne({ where: { user_id: user.id } });
  return !!account; // Return true if the account exists, false otherwise
}
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'check') {
      ...
      if (!userToCheck) {
        return interaction.reply('Please select a user to check.');
      }

      // Check if the selected user is the bot
      if (userToCheck.bot) {
        return interaction.reply('Bots do not have accounts.');
      }

      const hasAccount = await checkAccount(userToCheck);

      if (hasAccount) {
        return interaction.reply(`${userToCheck.tag} has an account.`);
      } else {
        return interaction.reply(`${userToCheck.tag} does not have an account.`);
      }
    } else {
      // Handle the existing account logic
