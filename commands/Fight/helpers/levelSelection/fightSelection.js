...
async function selectFight(interaction, raidId) {
  try {
    ...
    const filter = i => i.customId === 'fightSelect' && i.user.id === interaction.user.id;
    ...
    return new Promise(resolve => {
      collector.on('collect', async i => {
        ...
        // Directly query the Enemy model
        const enemy = await Enemy.findByPk(selectedFight.enemy_id);
        console.log(selectedFight.enemy_id)
        console.log(enemy)
    ...
    interaction.reply('Something went wrong while fetching fights.');
    return null;
  }
}
