...
class CharacterInstance {
  static async initCharacter(masterCharacterId, userId, characterId) {
    try {
      ...
      const enhancements_health_modifier = base_health * enhancements_health_bonus;
      ...
      const enhancements_damage_modifier = base_damage * enhancements_damage_bonus;
  ...
  static async initialize(characterId) {
    try {
      const characterData = await Character.findByPk(characterId);
      if (!characterData) throw new Error('Character not found');

      const masterCharacterId = characterData.dataValues.master_character_id;
      const userId = characterData.dataValues.user_id;

      await this.initCharacter(masterCharacterId, userId, characterId);
    } catch (error) {
      console.error(error);
